...
namespace TicketingSystem.Services
{
    ...
    public class PaymobService : IPaymobService
    {
        ...
        private string IntegrationId => _configuration["PaymobSettings:IntegrationId"] ?? throw new InvalidOperationException("Paymob Integration ID not configured");
        private string HmacSecret => _configuration["PaymobSettings:Hmac"] ?? throw new InvalidOperationException("Paymob HMAC secret not configured");
        ...
        public async Task<string> InitiatePaymentAsync(Order order, string? callbackUrl = null, string? returnUrl = null)
        {
            try
            {
                ...
                var paymobOrder = await CreatePaymobOrderAsync(authToken, order);

                // Step 3: Get payment key
                var paymentKey = await GetPaymentKeyAsync(authToken, paymobOrder.Id, order);
                ...
                //https://ksa.paymob.com/api/acceptance/iframes/9488?payment_token={paymentKey}
            ...
            {
                _logger.LogError(ex, "Error initiating Paymob payment for order {OrderId}", order.Id);
        ...
        public async Task<string> InitiatePaymentAsync(MemberShip membership, string? callbackUrl = null, string? returnUrl = null)
        {
            try
            {
                ...
                var paymobOrder = await CreatePaymobMemberAsync(authToken, membership);

                // Step 3: Get payment key
                var paymentKey = await GetPaymentMemberKeyAsync(authToken, paymobOrder.Id, membership);
        ...
        private async Task<PaymobOrderResponse> CreatePaymobMemberAsync(string authToken, MemberShip membership, string? callbackUrl = "https://ef0318b2efaa.ngrok-free.app/api/Transactions/paymob/GenericConfirm")
        {
        ...
        private async Task<string> GetPaymentMemberKeyAsync(string authToken, int paymobOrderId, MemberShip membership)
        {
        ...
        private async Task<PaymobOrderResponse> CreatePaymobOrderAsync(string authToken, Order membership, string? callbackUrl = "https://localhost:7150/api/Transactions/paymob/GenericConfirm")
        {
            var orderRequest = new
            {
                auth_token = authToken,
                delivery_needed = false,
                amount_cents = (int)(membership.FinalAmount * 100),
                currency = "SAR",
                merchant_order_id = $"ORD/{Guid.NewGuid()}/{membership.Id}",
                callback_url = callbackUrl ?? _configuration["PaymobSettings:CallbackUrl"],
                items = new[]
                {
            new
            {
                name = "Order",
                amount_cents = (int)(membership.FinalAmount * 100),
                description = $"Order Number: {membership.Id}-{membership.UserId}, CreatedAt: {membership.CreatedAt:yyyy-MM-dd}",
                quantity = 1
            }
        }
            };
        ...
        private async Task<string> GetPaymentKeyAsync(string authToken, int paymobOrderId, Order membership)
        {
            if (!int.TryParse(IntegrationId, out var integrationId))
            {
                throw new InvalidOperationException("Invalid IntegrationId format");
    ...
    public interface IPaymobService
    {
        ...
        Task<string> InitiatePaymentAsync(MemberShip order, string? callbackUrl = null, string? returnUrl = null);
    }
